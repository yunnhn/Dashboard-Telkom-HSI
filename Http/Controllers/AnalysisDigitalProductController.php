...
class AnalysisDigitalProductController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $targets = Target::where('segment', $selectedSegment)->where('period', $reportPeriod->format('Y-m-d'))->get()->keyBy(fn ($item) => $item->nama_witel . '_' . $item->metric_type . '_' . $item->product_name);
        ...
        $inProgressData = DocumentData::where('status_wfm', 'in progress')->where('segment', $selectedSegment)->whereYear('order_created_date', $inProgressYear)->select('order_id', 'milestone', 'segment', 'order_status_n', 'product as product_name', 'nama_witel', 'customer_name', 'order_created_date')->orderBy('order_created_date', 'desc')->get();
        $historyData = DocumentData::where('status_wfm', 'done close bima')->orderBy('updated_at', 'desc')->take(10)->get();
        ...
        return Inertia::render('AnalysisDigitalProduct', [ 'reportData' => $reportData ? $reportData->values()->all() : [], 'currentSegment' => $selectedSegment, 'period' => $periodInput, 'inProgressData' => $inProgressData, 'historyData' => $historyData, 'kpiData' => $kpiData, 'currentInProgressYear' => $inProgressYear, ]);
    }
